...
EditorDocument *editor_create(void) {
    ...
    if (!doc->gap.buf) { free(doc); return NULL; }
...
bool editor_insert(EditorDocument *doc, const char *text, size_t length) {
    ...
    if (doc->history.top < UNDO_STACK_MAX_DEPTH - 1) {
        ...
        cmd->type = CMD_INSERT;
        cmd->position = doc->gap.gap_start - length;
        cmd->length = length;
...
bool editor_delete(EditorDocument *doc, size_t position, size_t count) {
    ...
    size_t text_len = editor_get_length(doc);
    if (position + count > text_len) return false;
...
char *editor_get_text(const EditorDocument *doc) {
    ...
    memcpy(result, doc->gap.buf, before);
    memcpy(result + before, doc->gap.buf + doc->gap.gap_end, after);
    result[total] = '\0';
    return result;
}
...
bool editor_undo(EditorDocument *doc) {
    ...
    if (cmd->type == CMD_INSERT) {
        ...
        gap_buffer_move_gap(&doc->gap, cmd->position);
        memcpy(doc->gap.buf + doc->gap.gap_start, cmd->text, cmd->length);
...
void editor_compute_stats(const EditorDocument *doc, DocStats *stats) {
    ...
    for (size_t i = 0; text[i]; i++) {
        if ((text[i] & 0xC0) != 0x80) stats->char_count++;
        if (text[i] == '\n') {
            stats->line_count++;
            if (prev_newline) stats->paragraph_count++;
            prev_newline = true;
        } else { prev_newline = false; }
        if (text[i]==' '||text[i]=='\n'||text[i]=='\t') { in_word=false; }
        else if (!in_word) { in_word=true; stats->word_count++; }
    }
