...
void scintilla_configure_defaults(HWND sci) {
    ...
    /* Configuration des indicateurs (NLP) */
    // 0: Orthographe (rouge ondulé)
    SendMessageA(sci, SCI_INDICSETSTYLE, 0, INDIC_SQUIGGLE);
    SendMessageA(sci, SCI_INDICSETFORE,  0, RGB(255, 0, 0));

    // 1: Grammaire (bleu ondulé)
    SendMessageA(sci, SCI_INDICSETSTYLE, 1, INDIC_SQUIGGLE);
    SendMessageA(sci, SCI_INDICSETFORE,  1, RGB(0, 0, 255));

    // 2: Style (vert ondulé)
    SendMessageA(sci, SCI_INDICSETSTYLE, 2, INDIC_SQUIGGLE);
    SendMessageA(sci, SCI_INDICSETFORE,  2, RGB(0, 128, 0));
}
